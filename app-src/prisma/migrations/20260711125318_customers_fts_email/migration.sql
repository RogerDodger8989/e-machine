-- customers_fts måste byggas om för att indexera email — SQLite FTS5 stödjer
...
INSERT INTO customers_fts(entity_id, name, company, phone_normalized, email)
SELECT id, name, coalesce(company, ''), coalesce(phone_normalized, ''), coalesce(email, '') FROM customers;

CREATE TRIGGER customers_fts_ai AFTER INSERT ON customers BEGIN
  INSERT INTO customers_fts(entity_id, name, company, phone_normalized, email)
  VALUES (new.id, new.name, coalesce(new.company, ''), coalesce(new.phone_normalized, ''), coalesce(new.email, ''));
END;
